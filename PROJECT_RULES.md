# Project Rules

## Scope

- Develop the novel first; screenplay adaptation is a possible later project.
- Target 85,000 to 100,000 words across roughly 30 to 40 chapters.
- Do not draft the full novel until explicitly requested.
- Treat `docs/COMPLETE_PROJECT_BRIEF.md` as the controlling brief.

## Prose

- Use close third-person exclusively with Brooke Bennett, including the prologue. Found media broadens the visible world without changing narrative viewpoint. The final reader-only journal image is the sole controlled exception.
- Write normal commercial novel prose with full scenes, natural paragraphs, and age-appropriate dialogue.
- Keep found media to roughly 10 to 15 percent of the novel.
- Brooke has no romantic subplot.
- Brooke and Claire are ordinary sisters, neither unusually close nor distant. Their beach memory anchors Claire's later breakthrough.
- Avoid screenplay formatting, transcript-heavy presentation, creepypasta rhythms, frequent fragments, and excessive one-line paragraphs.
- Avoid em dashes and overused italics.
- Let readers notice implications. Do not overexplain fear or rely on cheap jump scares.
- Preserve ambiguity and do not force every chapter to end on a cliffhanger.

## Canon guardrails

- Brooke has no special bloodline, immunity, destiny, or inherited resistance.
- Anna is not Brooke's biological mother and is not searching for her.
- Anna wants closeness with Brooke and succeeds through patience and learned trust, not aggression.
- Do not fully explain the illness, whispers, origin, mechanism, cure, or shared knowledge.
- Do not give recovered people a formal category name.
- Society changes gradually while normal systems continue operating.
- Animals detect the change before people do.
- No changed person can knowingly state a direct falsehood. They may omit, redirect, answer narrowly, and exploit assumptions.
- Touch does not transmit the illness, but the actual route remains unexplained and unknown to characters.
- Anna uses the keypad code visible in an old family video to enter the Bennett home.
- Anna takes Brooke's old journal, and the journal is the final proof that she studied Brooke.
- Mark is away for work, never returns, and remains of unknown fate.
- Claire never hurts Cooper.
- Anna enters while Brooke is at school and may alter only believable presentation details, never her body.
- Anna never directly claims motherhood, and Brooke is not uniquely important to her.
- Everyone who experiences the purge eventually changes while continuing to sleep, eat, drink, and function.
- School remains open, and public institutions shift gradually toward preventing separation.
- The ending does not rely only on a reflective green-eye effect.

## Continuity practice

- Label undecided material as open rather than silently making it canon.
- Track setup, echo, and payoff for every twist clue.
- Preserve emotional plausibility: Claire must be lovable, Sarah genuinely loving but imperfect, and Anna deeply trustworthy.
- Update the bible, outline, and changelog whenever a decision becomes locked.
